...
if [ -n "$REACT_APP_BACKEND" ]; then
  sed -i "s|http://52.202.56.186:3000|$REACT_APP_BACKEND|g" /usr/share/nginx/html/static/js/*.js
else
  echo "REACT_APP_BACKEND environment variable not found."
fi

if [ -n "$REACT_APP_STAT_URL" ]; then
  sed -i "s|http://52.202.56.186:3100|$REACT_APP_STAT_URL|g" /usr/share/nginx/html/static/js/*.js
else
